...
TIMESTAMPS = ["0.04", "1.0", "10.0", "60.0", "600.0", "3600.0", "equilibrium", "eq"] #TODO: "eq"
...
def plots(input_file):
    ...
    plt.plot([d[ts]["nat"][ms]["mean"] for ts in TIMESTAMPS], label="normal")
    plt.plot([d[ts]["nat_rev"][ms]["mean"] for ts in TIMESTAMPS], label="reversed")
    plt.title("Time series - MCC mean")
    plt.xlabel("Time [sec]")
    plt.xticks(np.arange(len(TIMESTAMPS)), TIMESTAMPS)
    plt.ylabel("MCC")
    plt.legend()
    plt.savefig("tRNA_time_series.png")
    plt.close()

    ternary_plot(input_file)


def combined_plots(rna_classes=["5s", "16s", "23s", "grpI", "grpII", "RNaseP", "srp", "telomerase", "tmRNA", "tRNA"], TS="1.0", MS="MCC"):
    d = {}
    for rna_class in rna_classes:
        with open(f"analysis_results/{rna_class}/{rna_class}.JSON") as f:
            db = json.load(f)
