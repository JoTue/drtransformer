...
def metrics(n, ss1=None, ss2=None, bpp1=None, bpp2=None):
    ...
    try:
        ...
        MCC = float('nan')  # TODO better?: MCC = 0
    try:
...
def analyze(dataset, id1, id_end, ids_per_job, tmp_dir, t_list, t_ext, directory):
    ...
    with open(f"{dataset}") as data_f:
        ...
        while line:
            if line.startswith(">"):
                ...
                if not os.path.isfile(f"../{directory}/{dataset_name}/{header}/{header}.log") or not os.path.isfile(f"../{directory}/{dataset_name}/{header}_rev/{header}_rev.log"):
                    incomplete_seq_count += 1
                    if not os.path.isfile(f"../{directory}/{dataset_name}/{header}/{header}.log"):
                        missing_files.append(f"{header}.log")
                        missing_file_count += 1
                    if not os.path.isfile(f"../{directory}/{dataset_name}/{header}_rev/{header}_rev.log"):
                        missing_files.append(f"{header}_rev.log")
                        missing_file_count += 1
                ...
                for mode in [0, 1]:
                    with open(f"../{directory}/{dataset_name}/{header}{['', '_rev'][mode]}/{header}{['', '_rev'][mode]}.log", "rb") as f:  # parse log-file for equilibrium occupancies (ignore t-end occupancies)
                        lines = f.readlines()
                        d = {}
                        try:
                            for line in lines[::-1]:
                            ...
                            print("Error", f"../{directory}/{dataset_name}/{header}{['', '_rev'][mode]}/{header}{['', '_rev'][mode]}.log")
                            erroneous_files.append(f"{header}{['', '_rev'][mode]}.log")
                            erroneous_file_count += 1
                            error_f = True
                            break
                        dist_nat = 0
                        dist_natc = 0
                        ...
                        for el in d:
                            ...
                            if d[el][0] + d[el][1] == 0: # continue if not populated
                                continue
                            dist_nat = RNA.bp_distance(db[header]["nat_ss"], el)  # TODO: Error here ? bpdis for equilibrium seems to be too high
                    ...
                    with open(f"../{directory}/{dataset_name}/{header}{['', '_rev'][mode]}/{header}{['', '_rev'][mode]}.drf", "rb") as f:  # parse drf-file for occupancies at time points given by t-list
                        ...
                        for line in lines[::-1]:
                            ...
                            try:
                                ...
                                print("Error", f"../{directory}/{dataset_name}/{header}{['', '_rev'][mode]}/{header}{['', '_rev'][mode]}.drf")
                                erroneous_files.append(f"{header}{['', '_rev'][mode]}.drf")
                                erroneous_file_count += 1
                                error_f = True
                                break
                ...
                if not error_f:
                    with open(f"../{directory}/analysis_results/{dataset_name}/{header}.json", "w") as json_f:
                        print(json.dumps(db, indent=4), file=json_f)
                else:
                    print("Omitting writing json file.")
    ...
    print(f"{erroneous_files=}")
    print(f"{erroneous_file_count=}")
...
def main():
    ...
    analyze(args.dataset, args.id1, args.id_end, args.ids_per_job, args.tmp, args.t_list, args.t_ext, args.directory)
